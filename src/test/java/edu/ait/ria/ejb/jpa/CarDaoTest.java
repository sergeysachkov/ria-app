...
import edu.ait.ria.ejb.service.CarService;
...
public class CarDaoTest {
    ...
    public void testCreateCar() {
        ...
        carDao.addCar(car);
        List<Car> testCars = carDao.getCarsByFieldValue("model", model);
        assertTrue(testCars.size() == 1);
        assertEquals(testCars.get(0), car);
        carDao.deleteCar(testCars.get(0).getId());
    }
    ...
    public void testDeleteCar() {
        ...
        assertEquals(testCars.get(0), car);
        carDao.deleteCar(testCars.get(0).getId());
        testCars = carDao.getCarsByFieldValue("model", model);
    ...
    public void testGetCar() {
        ...
        assertTrue(testCars.size() == 1);
        assertEquals(testCars.get(0), car);
        Car getCar = carDao.getCar(testCars.get(0).getId());
    ...
    public void testGetCars() {
        ...
        carDao.addCar(car);
        carDao.addCar(car1);
        List<Car> testCars = carDao.getCars();
        assertTrue(testCars.size() > 0);
        Car foundCar = null;
        Car foundCar1 = null;
        for(Car c : testCars){
        ...
        carDao.deleteCar(foundCar.getId());
        carDao.deleteCar(foundCar1.getId());
    }
}
